...
\paragraph*{Un caso gi\`a noto: Composizione di Stacksort con Bubblesort} Si considera dunque la composizione $SB = S\circ{B}$ e ci si chiede quali permutazioni possano essere ordinate da essa.$$(S\circ B)^{-1}Av(21)=B^{-1}S^{-1}(Av(21))=B^{-1}(Av(231))$$
...
\end{description}
\section{Composizioni di un operatore di ordinamento con bubblesort}
...
\subsection{Composizione di queuesort con bubblesort}
...
\section{Composizioni di un operatore di ordinamento con \textit{queuesort}}
...
\subsection{POP-Queuesort - dettagli ulteriori}L'algoritmo POP-queuesort richiede una diversa analisi rispetto agli altri algoritmi.\\
...
\end{algorithmic}
\end{algorithm}
La dimostrazione che \textit{Cons} (cos\'i come \textit{Min}) sia un algoritmo ottimale nella classe degli algoritmi \textit{POP-queuesort} segue dal fatto che esso ordina tutte e sole le sequenze dell'insieme Av(321, 2413), che sono esattamente tutte e sole le permutazioni ordinabili con una coda POP\cite{cioni2021sorting}.
...
\begin{center}
\mmpattern{scale=1.5}{3}{1/3,2/1,3/2}{}{1/3/2/4/1}
\end{center}
...
\begin{center}
\mmpattern{scale=1.5}{3}{1/3,2/2,3/1}{2/2,2/3}{1/3/2/4/1}$\Rightarrow$\mmpattern{scale=1.5}{4}{1/3,2/4,3/2,4/1}{3/2,3/3,3/4}{}
\end{center}
...
\begin{center}
\mmpattern{scale=1.5}{4}{1/3,2/4,3/2,4/1}{3/3}{} $= 352\overline{4}1$
\end{center}
...
\begin{center}\mmpattern{scale=1.5}{4}{1/2,2/4,3/1,4/3}{}{2/4/3/5/1}\end{center} che soddisfa anche la condizione per l'inversione $(4,3)$, il mesh-pattern cos\`i ottenuto equivale al pattern classico 24513, che \`e l'unica controimmagine che si possa ricavare da questo candidato.
...
\decpatternww{scale=1.7}{4}{1/2,3/1,4/3}{}{}{}{1/3/3/5/{\mpattern{scale=0.7}{2}{2/1,1/2}{}}}{}.
\end{center}
...
\subsection{Osservazione sulle analisi di composizioni di operatori di ordinamento con POP-queuesort } 
...
Se la forma di $\pi$ \`e tale che $b$ entri in coda, per mentenere la sottosequenza $bca$ \`e necessario che $c$ non sia un LTR massimo. Questa condizione \`e soddisfatta se $\pi$ contiene un'occorrenza del mesh-pattern \mmpattern{scale=0.7}{3}{1/2,2/3,3/1}{}{0/3/2/4/1}, ovvero se contiene uno dei pattern classici 4231 o 2431.\\\\
